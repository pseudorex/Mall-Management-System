...
def dataAnalysis(dbname):
    ...
    #-----------
    #rootDA = Tk()
    #do not create Tk() window as when called from within another .py file with Tk()
    #it causes pyimage and panadastable to misbehave so use Toplevel()
    #when testing as standalone, Toplevel() like messagebox creates and opens Tk() automatically, should not be a problem
    rootDA = Toplevel()

    rootDA.title('My Python Project')
    #rootDA.geometry('1024x680')
    #Maximize rootDA window with title
    ...
    #----------------------------------------
    '''
    #label frame - frame2/display statistical details using dataframe
    frame2 = LabelFrame(frameControls, text='frame2', font=('Times', 14), bd=5, relief=RIDGE)
    #frame2.grid(row=0, column=1, sticky=W, padx=20, pady=20)
    frame2.grid(row=1, column=0, sticky=W, padx=20, pady=20)
    r=0
    Label(frame2, text="").grid(row=r, column=0, sticky=W, padx=4, pady=2)
    r+=1
    Label(frame2, text="STATISTICAL DETAILS OF DF DATA").grid(row=r, column=0, columnspan=2, sticky='W')
    r+=1
    ...
    r+=1
    Button(frame2, text='STAT DETAILS', command=statDetailBtnClickEvent).grid(row=r, column=1)
    r+=1;
    Label(frame2, text="").grid(row=r, column=2, sticky=W, padx=4, pady=2)
    '''
    #----------------------------------------
    ...
    #----------------------------------------
    #label frame - frame4/plot dataframe data
    frame4 = LabelFrame(frameControls, text='frame4', font=('Times', 14), bd=5, relief=RIDGE)
    frame4.grid(row=0, column=3, sticky=W, padx=20, pady=20)
    r=0
    Label(frame4, text="").grid(row=r, column=0, sticky=W, padx=4, pady=2)
    r+=1
    Label(frame4, text="PLOT DF DATA").grid(row=r, column=1, columnspan=2, sticky='W')
    r+=1
    Label(frame4, text="").grid(row=r, column=0, sticky='W') #for top & left padding
    r+=1
    Label(frame4, text="Select DF columns for X & Y axis of plot").grid(row=r, column=0, columnspan=3, sticky='W')
    #dataframe
    df=createDataFrame(cboTables.get())
    r+=1
    #X-axis
    Label(frame4, text="X-axis").grid(row=r, column=0, sticky='E')
    cboXaxis=ttk.Combobox(frame4, values=cboColumns['values'])
    cboXaxis.grid(row=r, column=1)
    cboXaxis.current(0)
    r+=1
    #Y-axis
    Label(frame4, text="Y-axis").grid(row=r, column=0, sticky='E')
    cboYaxis=ttk.Combobox(frame4, values=cboColumns['values'])
    cboYaxis.grid(row=r, column=1)
    cboYaxis.current(0)
    def drawPlotBtnClickEvent():
        df=createDataFrame(cboTables.get())
        if varLineBar.get()=='line':
            plt.plot(df[cboXaxis.get()],df[cboYaxis.get()])        
        else:
            plt.bar(df[cboXaxis.get()],df[cboYaxis.get()])
        plt.show()
    #radio button - line or bar chart
    r+=1
    Label(frame4, text="Select Plot Type").grid(row=r, column=0, sticky='W')
    varLineBar = StringVar()
    values = {"Line chart":"line","Bar chart":"bar"}
    for (text, value) in values.items():
        #Radiobutton(frameControls, text = text, variable = var, value = value, command=radiooption).grid(column=c, row=r, sticky=W)
        rbtplot = Radiobutton(frame4, text=text, variable=varLineBar, value=value, tristatevalue=0) #tristatevalue for initial state
        rbtplot.grid(row=r, column=1, sticky='W')
        r+=1
    r+=1
    Button(frame4, text='Draw Plot', command=drawPlotBtnClickEvent).grid(row=r, column=1, sticky='W')
    r+=1;
    Label(frame4, text="").grid(row=r, column=1, sticky=W, padx=4, pady=2)
    #----------------------------------------
